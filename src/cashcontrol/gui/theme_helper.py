...
def html_color(name: str) -> str:
    """Same as color() but intended for use inside HTML strings."""
    return color(name)


def html_style_block() -> str:
    """Return a <style> block for HTML content that adapts to current theme.

    Covers common elements: body, headings, links, code, tables, blockquotes.
    """
    return f"""<style>
...
blockquote {{
    ...
    border-left: 3px solid {color('warning_border')};
    padding: 6px 10px;
    color: {color('warning_text')};
    margin: 8px 0;
    border-radius: 4px;
}}
.status-ok {{ color: {color('success')}; }}
.status-err {{ color: {color('error')}; }}
.status-warn {{ color: {color('warning')}; }}
.status-info {{ color: {color('info')}; }}
</style>"""