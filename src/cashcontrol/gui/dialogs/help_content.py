...
<table>
  ...
  <tr><td><b>Закрыть вкладку</b></td><td>Кнопка <b>✕</b> на вкладке или <b>Ctrl+W</b></td></tr>
...
<table>
  ...
  <tr><td><b>Переустановка</b></td><td>Полная переустановка ПО</td><td>Открывает диалог</td></tr>
  <tr><td><b>Mover</b></td><td>Доставка файлов и команд</td><td>Открывает диалог</td></tr>
</table>
...
через <b>Редактор команд</b> (кнопка ✏ в боковой панели).</p>
"""
...
<p>Открывается кнопкой <b>🔗</b> в боковой панели.</p>
...
<p>Пути к внешним утилитам. Открывается кнопкой <b>📦</b> в боковой панели.</p>
...
<p>Открывается кнопкой <b>✏</b> в боковой панели. Позволяет создавать
...
<p>Открывается кнопкой <b>🕐</b> в боковой панели. Окно не модальное —
...
def _page_reinstall() -> str:
    return get_css() + """
<h1>Переустановка ПО</h1>

<p>Инструмент для полной переустановки кассового программного обеспечения.
Открывается кнопкой <b>«Переустановка»</b> в панели инструментов.</p>

<h2>Возможности</h2>
<ul>
  <li><b>Просмотр архивов</b> — список доступных архивов ПО по типу кассы (pos, sco3, touch)</li>
  <li><b>Извлечение из ISO</b> — загрузка ISO-образа и автоматическое извлечение .tar архива</li>
  <li><b>Установка на кассу</b> — пошаговый процесс с прогрессом и логом</li>
</ul>

<h2>Расположение архивов</h2>
<table>
  <tr><td><code>soft/reinstall/pos/</code></td><td>Архивы для POS-касс</td></tr>
  <tr><td><code>soft/reinstall/sco3/</code></td><td>Архивы для SCO v3</td></tr>
  <tr><td><code>soft/reinstall/touch/</code></td><td>Архивы для touch-касс</td></tr>
</table>

<h2>Процесс установки</h2>
<ol>
  <li>Загрузка архива на кассу через SSH</li>
  <li>Остановка ПО: <code>cash stop</code> + <code>killall java</code></li>
  <li>Очистка баз данных и директории ПО</li>
  <li>Распаковка нового архива</li>
  <li>Установка прав (для sco3)</li>
  <li>Перезагрузка: <code>cash reboot</code></li>
</ol>

<div class="warn">Переустановка удаляет все данные кассового ПО. Убедитесь,
что касса не обслуживает покупателей.</div>
"""


def _page_mover() -> str:
    return get_css() + """
<h1>Mover — доставка файлов и команд</h1>

<p>Mover автоматизирует настройку кассы после переустановки ПО: загрузку конфигов,
выполнение SSH-команд, применение данных Loymax и QRID.
Открывается кнопкой <b>«Mover»</b> в панели инструментов.</p>

<h2>Сценарии</h2>
<p>Mover работает со <b>сценариями</b> — JSON-файлами, описывающими последовательность шагов.
Сценарии хранятся в папке <code>mover/scenarios/</code>.</p>

<p>Программа поставляется с дефолтным сценарием, который можно редактировать или
создать новый через встроенный конструктор.</p>

<h2>Типы шагов</h2>
<table>
  <tr><th>Тип</th><th>Описание</th></tr>
  <tr><td>Загрузка файлов</td><td>Загрузка файлов на кассу через SCP. Список файлов зависит от типа кассы.</td></tr>
  <tr><td>Выполнение команд</td><td>SSH-команды по типу кассы. Для touch поддерживаются подтипы (wide-kya, quad-baton и т.д.).</td></tr>
  <tr><td>Настройка Loymax</td><td>Поиск логина/пароля в CSV-файле по имени магазина и номеру кассы, запись в loymax.properties.</td></tr>
  <tr><td>Настройка QRID</td><td>Поиск QRID в CSV-файле и подстановка в bank-gazprom_sbp-config.xml. Только для POS.</td></tr>
  <tr><td>Настройка COM-порта</td><td>Установка COM-порта банковского терминала в pinpad.ini. Только для POS.</td></tr>
</table>

<h2>Конструктор сценариев</h2>
<p>Встроенный конструктор позволяет создавать и редактировать сценарии
через визуальный интерфейс:</p>
<ul>
  <li><b>Палитра блоков</b> — перетаскивайте типы шагов из левой панели в список</li>
  <li><b>Список шагов</b> — перетаскивание для изменения порядка, кнопки вверх/вниз и удаление</li>
  <li><b>Настройки шага</b> — правая панель для редактирования параметров каждого шага</li>
</ul>
<p>Открывается кнопками ✏ (редактировать) и ＋ (создать) рядом с выбором сценария.</p>
"""
...
_TREE = [
    ...
    ("Переустановка ПО",   _page_reinstall),
    ("Mover",              _page_mover),
